...
kubectl exec -t -i cycloid-vault-0 -- vault operator init -status
VAULT_INIT_STATUS=$?
...
if [ $VAULT_INIT_STATUS -eq 2 ]; then
  ...
  kubectl exec -t -i cycloid-vault-0 -- vault operator init -format=json | tee "$OUTPUT_DIR/vault-init.json"
