...
if [ $VAULT_SEAL_STATUS -eq 2 ]; then
  ...
  if [ $JQ_STATUS -eq 0 ] && [ -f "$OUTPUT_DIR/vault-init.json" ]; then
    kubectl -n "$NAMESPACE" exec -t -i cycloid-vault-0 -- vault operator unseal "$(jq -r '.unseal_keys_b64[0]' "$OUTPUT_DIR/vault-init.json")"
  else
  ...
  echo ">> Key 2"
  if [ $JQ_STATUS -eq 0 ] && [ -f "$OUTPUT_DIR/vault-init.json" ]; then
    kubectl -n "$NAMESPACE" exec -t -i cycloid-vault-0 -- vault operator unseal "$(jq -r '.unseal_keys_b64[1]' "$OUTPUT_DIR/vault-init.json")"
  else
    kubectl -n "$NAMESPACE" exec -t -i cycloid-vault-0 -- vault operator unseal
  fi
  ...
  echo ">> Key 3"
  if [ $JQ_STATUS -eq 0 ] && [ -f "$OUTPUT_DIR/vault-init.json" ]; then
    kubectl -n "$NAMESPACE" exec -t -i cycloid-vault-0 -- vault operator unseal "$(jq -r '.unseal_keys_b64[2]' "$OUTPUT_DIR/vault-init.json")"
  else
    kubectl -n "$NAMESPACE" exec -t -i cycloid-vault-0 -- vault operator unseal
  fi
